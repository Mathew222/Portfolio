        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24">
            ...
            <div className="flex flex-col justify-between">
                ...
                <div className={`mt-8 md:mt-12 space-y-2 transition-all duration-1000 delay-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
                    ...
                    <p className="text-base md:text-lg font-medium text-gray-500 dark:text-gray-400">{RESUME_DATA.location}</p>
                </div>
            </div>
            ...
            <div className={`bg-zinc-50 dark:bg-zinc-900 p-6 md:p-12 border border-black dark:border-white/20 relative transition-all duration-1000 delay-300 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-20'}`}>
                ...
                <form className="space-y-6 md:space-y-8" onSubmit={(e) => e.preventDefault()}>
                    <div className="space-y-1">
                        ...
                        <input type="text" id="name" className="w-full bg-transparent border-b-2 border-gray-300 dark:border-zinc-700 py-3 text-black dark:text-white text-lg focus:outline-none focus:border-black dark:focus:border-white transition-colors rounded-none placeholder-gray-400 font-display" placeholder="YOUR NAME" />
                    </div>
                    <div className="space-y-1">
                        <label htmlFor="email" className="text-xs font-bold uppercase tracking-widest text-black dark:text-white">Email</label>
                        <input type="email" id="email" className="w-full bg-transparent border-b-2 border-gray-300 dark:border-zinc-700 py-3 text-black dark:text-white text-lg focus:outline-none focus:border-black dark:focus:border-white transition-colors rounded-none placeholder-gray-400 font-display" placeholder="YOUR@EMAIL.COM" />
                    </div>
                    <div className="space-y-1">
                        <label htmlFor="message" className="text-xs font-bold uppercase tracking-widest text-black dark:text-white">Message</label>
                        <textarea id="message" rows={4} className="w-full bg-transparent border-b-2 border-gray-300 dark:border-zinc-700 py-3 text-black dark:text-white text-lg focus:outline-none focus:border-black dark:focus:border-white transition-colors rounded-none placeholder-gray-400 font-display resize-none" placeholder="TELL ME ABOUT YOUR PROJECT"></textarea>
                    </div>
